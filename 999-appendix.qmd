...
A dependable visualization script separates five concerns. This separation also supports the general reproducibility principle that every result should be traceable from recorded inputs through documented computational steps [@sandve2013tenrules].
...
Begin with the analytical question, not a preferred library or chart type. A useful design process connects the reader's task, the available data, the chosen visual encoding, and the interaction or presentation method [@munzner2014visualization].
...
For precise quantitative comparison, prefer encodings roughly in this order. The ordering reflects classic experimental work showing that graphical encodings differ in how accurately people can judge quantitative values [@cleveland1984graphical]:
...
The grammar-of-graphics workflow builds a chart by combining data, mappings, geometric marks, scales, coordinates, facets, and a theme [@wilkinson2005grammar]:
...
Color choices can change what patterns readers perceive and can even introduce visual artifacts. Prefer perceptually uniform, data-appropriate palettes and test them under realistic viewing conditions [@crameri2020misuse]. For interactive outputs, treat the Web Content Accessibility Guidelines as a baseline for contrast, keyboard access, focus visibility, and non-text alternatives [@w3c2023wcag22].
...
- @munzner2014visualization provides a systematic framework for matching visual designs to data and analytical tasks.
- @cleveland1984graphical supplies foundational empirical evidence for comparing quantitative encodings.
- @wilkinson2005grammar develops the compositional theory behind grammar-of-graphics libraries.
- @crameri2020misuse explains common scientific color failures and principles for safer palette selection.
- @w3c2023wcag22 is the normative accessibility standard for web-delivered figures and interactive views.
- @sandve2013tenrules gives concise, durable rules for reproducible computational research.
